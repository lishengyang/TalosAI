...
#define MEMORY_SIZE (1024 * 512 * 1024) // 1G内存大小
...
int main() {
    // 分配内存
    struct Data* data = (struct Data*)mmap(NULL, MEMORY_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_LOCKED, -1, 0);
    if (data == MAP_FAILED) {
