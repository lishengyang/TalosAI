...
def extract_code(response, code_queue):
   ...
   # code_block = re.findall(r'```python([\s\S]*?)```', response)
    code_block = re.findall(r'```[\s\S]*?([\s\S]*?)```', response)
...
prompt = "i am building a code demo for testing pwm on my arm board which is the raspberry pi 4,show me the steps"
response = ask_GPT(system_message, prompt)
print(response)

# Extract target name from prompt
target_name = re.search(r"testing (.*?) on", prompt)
if target_name:
    target_name = target_name.group(1).replace(" ", "_")
else:
    target_name = "unknown_target"

filename = f"{target_name}_code.py"
