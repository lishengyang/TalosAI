...
Some demos that use gpt4 and gpt3.5 to control the hardware,on the embedded boards like raspberry pi,these are the demo code that is generated automaticaly by GPT,
...
Show what you can accomplish with LLMs in an embedded hardware platform. LLMs can work out most of the design goals with prompts in Openai APIs or other LLMs,but most of them are not one-shot-ok ,most of these auto codes need to be modified and recoded ,re-generated by LLMs,and faults-pass-to-AI is a very interesting process during the debugging. well ,it really does not take too much time passing the fault messages into the prompts windows ,right?
The process?
First,i look for the real life design goals on the boards ,focusing on hardware controlling ,driving all kinds of peripherals boards and sensor boards,
when you are using a robot (which can be programmed of course),you just want it to work as fast as possible,so just use LLMs ! They really works!
